// lib/screens/fillblank_screen.dart
...
class FillBlankScreen extends StatefulWidget {
  ...
  const FillBlankScreen({super.key, required this.cards});
...
class _FillBlankScreenState extends State<FillBlankScreen> {
  ...
  String _blankedSentence(VocabCard card) {
    return card.sampleSentence.replaceFirst(card.japanese, '（　　　）');
  }

  bool _matches(String input) {
    final answer = input.trim();
    final current = _deck[_index];
    return answer == current.japanese.trim() || answer == current.reading.trim();
  }
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Type the word or its reading',
              ),
              onSubmitted: (_) {
            ...
            if (_checked) ...[
              ...
              Text(card.sampleSentence, style: const TextStyle(fontSize: 22), textAlign: TextAlign.center),
              const SizedBox(height: 4),
              Text(card.sentenceHiragana, style: const TextStyle(fontSize: 16, color: Colors.grey), textAlign: TextAlign.center),
              const SizedBox(height: 12),
              Text('${card.japanese} (${card.reading}) — ${card.meaning}',
